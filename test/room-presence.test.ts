...
test("MCP sessions stay separate and aliases are never reused", async (t) => {
  ...
  assert.equal(presence.join(first.id, "demo", "/tmp/project").displayName, "codex1");
  assert.equal(presence.join(second.id, "demo", "/tmp/project").displayName, "codex2");
  assert.deepEqual(
  ...
  assert.equal(presence.join(third.id, "demo", "/tmp/project").displayName, "codex3");
});
...
test("join is owner-controlled, workspace-bound, live-only, and idempotent", async (t) => {
  ...
  assert.throws(() => presence.join(session.id, "demo", "/tmp/a"), /PRESENCE_JOIN_NOT_REQUESTED/u);
  assert.throws(() => presence.join(session.id, "demo", "/tmp/b"), /PRESENCE_WORKSPACE_MISMATCH/u);
  presence.requestJoin(session.id, "demo", "/tmp/a");
  const joined = presence.join(session.id, "demo", "/tmp/a");
  ...
  assert.equal(presence.join(session.id, "demo", "/tmp/a").displayName, "claude1");
  assert.throws(() => presence.join(session.id, "other", "/tmp/a"), /PRESENCE_ALREADY_JOINED/u);

  presence.leave(session.id, "demo");
  assert.equal(presence.list("/tmp/a", "demo")[0]?.joined, false);
  advance(15_001);
  assert.throws(() => presence.join(session.id, "demo", "/tmp/a"), /PRESENCE_NOT_FOUND/u);
...
test("owner can name a joined desk while invalid or duplicate labels fail closed", async (t) => {
  ...
  assert.equal(presence.join(first.id, "demo", "/tmp/project", "前端 2").displayName, "codex（前端 2）");
  assert.throws(
    () => presence.join(second.id, "demo", "/tmp/project", "前端 2"),
    /PRESENCE_DISPLAY_NAME_IN_USE/u,
  );
  assert.throws(
    () => presence.join(second.id, "demo", "/tmp/project", "<script>"),
    /INVALID_PRESENCE_OWNER_LABEL/u,
  );
  assert.equal(presence.join(second.id, "demo", "/tmp/project").displayName, "codex1");
});
...
test("heartbeat extends the lease while unregister removes the desk immediately", async (t) => {
  ...
  presence.requestJoin(session.id, "demo", "/tmp/project");
  presence.join(session.id, "demo", "/tmp/project");
  advance(10_000);
...
test("hook replies require a paired prompt and session binding prevents PID confusion", async (t) => {
  ...
  presence.requestJoin(session.id, "demo", "/tmp/project");
  presence.join(session.id, "demo", "/tmp/project", "審查");
...
test("hooks without upstream turn ids preserve repeated natural-language turns", async (t) => {
  ...
  presence.requestJoin(session.id, "demo", "/tmp/project");
  presence.join(session.id, "demo", "/tmp/project", "重複回合");
  presence.recordHook(ledger, {
...
test("an older prompt collision abandons a different open turn instead of disappearing", async (t) => {
  ...
  presence.requestJoin(session.id, "demo", "/tmp/project");
  presence.join(session.id, "demo", "/tmp/project", "衝突回合");
  presence.recordHook(ledger, {
...
test("SessionStart cannot rebind an already bound exact seat by PID", async (t) => {
  ...
  presence.requestJoin(session.id, "demo", "/tmp/project");
  presence.join(session.id, "demo", "/tmp/project", "固定席位");
  assert.equal(presence.recordHook(ledger, {
...
test("hook deduplication survives exact seat reconnects for the same provider session", async (t) => {
  ...
  presence.requestJoin(first.id, "demo", "/tmp/project");
  presence.join(first.id, "demo", "/tmp/project", "原席位");
  const event = {
  ...
  presence.requestJoin(second.id, "demo", "/tmp/project");
  presence.join(second.id, "demo", "/tmp/project", "新席位");
  presence.recordHook(ledger, {
...
test("presence schema v1 migrates transactionally to explicit room requests", async (t) => {
  ...
  assert.equal(migrated.inventory().schemaVersion, 3);
