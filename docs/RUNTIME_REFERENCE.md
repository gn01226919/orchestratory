...
這份檔案收錄原本寫在 `README.md` 裡的詳細 runtime 說明。搬過來的原因是篇幅：README 的工作是讓
第一次看到這個專案的人知道它是什麼、怎麼跑起來、以及它**不**保證什麼；逐項行為描述是查閱用的，
放在 README 中段只會把那三件事淹掉。

**內容逐字保留，未經改寫。** 因此其中的時態、「隔離開發分支」「尚未切換」等說法反映的是搬移
當下（2026-09）之前的撰寫時點，可能已經落後於程式碼。以程式碼與 `docs/DECISIONS.md` 為準；
`README.md` 只保留經過重新查證的敘述。
