...
- `#seatRoomMark` 與 `#seatWroteSince` 都把讀取錯誤吞成「沒有證據」。這是刻意的（「看不到」不等於
  ...
  的空 catch 同族，一併列在這裡。 `hasAuthorMessageAfter` 改為對未知 room 拋錯之後，
  這個 catch 又多吞一種錯誤：資料庫損毀與「房間不存在」在呼叫端看起來一樣。政策本身不變，
  但失敗點多了一個，記在這裡。
...
## ADR-044：資料目錄可覆寫，覆寫要驗證；一個 store 失敗不該帶走不需要它的工具

**狀態：** 已實作（2026-09-04）。審查第一輪十條發現：六條已關閉、三條部分、一條（TOCTOU）未關閉且
明確記錄於下方。原本寫「已處置九條半」，那是把「已改過」當成「已關閉」——審查第二輪指出這句話
本身就是本專案反覆出現的那個毛病。

**問題。** 這台機器上所有 MCP session 同時死掉。三份已安裝 runtime 都是 `SCHEMA_VERSION = 5`，
而 4318 的 daemon 跑工作樹原始碼（schema 6）已把 `room-inbox.sqlite` 升到 `user_version = 6`。
舊 runtime 拒絕開啟**是正確的**——舊程式碼誤讀新資料比停下來更危險。錯的是**代價**：inbox 在
`CollaborationService` 建構子裡建立，它一拋錯就帶走整個服務，以及 27 個工具中**不需要 inbox 的
那 20 個**（19 個完全不受影響，`room_join_request` 只少一個 briefing 欄位）。

**這個數字原本寫 21，是錯的，而且錯得有代表性。** 它不是量出來的，是對一份清單做算術，而那份清單
漏掉三個經由服務層方法間接使用 inbox 的工具：`room_send`、`room_await_reply`、`list_agents`。當時的
「驗證」是掃 MCP 層原始碼裡有沒有 `#seatInbox()` 字樣，而那三個的依賴在別的檔案裡，所以掃不到。
詳見 ADR-045。

**決定一：降級的粒度。** `inbox` 保留為 getter，錯誤從「建構時」移到「呼叫時」；新增
`inboxAvailable`。**只有 schema-too-new 會降級**——這一條是審查第一輪指出的，我原本捕捉所有建構
錯誤，等於把「資料庫損毀」和「你的 build 舊了」當成同一件事處理，讓一個 fail-closed 的守衛悄悄
變成 fail-open。損毀、權限失敗仍然整個 fail closed。

**決定二：資料目錄可被 `ORCHESTRATORY_DATA_DIR` 覆寫。** 根因是 `defaultDataDirectory` 寫死一個
路徑、無法覆寫，所以開發工作樹與已安裝 runtime 共用同一份資料庫，前者的 migration 會單向鎖死
後者。**無效值拋錯而不退回預設**——靜默退回會重現正要修的那個失敗：開發者以為自己隔離了，其實
沒有，而他會在遷移正式資料庫時才發現。

**驗證方式被審查修正過一次，值得記。** 第一版只做字串比對，實測出兩個繞過：`/private/var/db`
通過而 `/var/db` 被擋（`/var` 是 symlink，清單裡只有一種拼法）；`/SYSTEM`、`/Etc`、`/USR/local`
全部通過（macOS 大小寫不敏感，它們就是被拒絕的那些目錄）。現在先 `realpath` 解析（目標不存在時
解析最深的既有祖先再接上其餘），再以大小寫折疊比對。

**修正過程中測試抓到一個真缺陷**：找最深祖先的迴圈用 `slice(parent.length + 1)`，而根目錄 `/`
的長度已包含分隔符，所以 `/etcetera/data` 被切成 `/tcetera/data`——**只在根目錄下一層才會出現**。
那條測試原本是為了驗證「名字開頭像保留字的目錄不該被誤擋」而寫的，順手抓到無關的 off-by-one。

**明確未關閉的殘留（不宣稱已處理）：**

- **TOCTOU。** 解析路徑是一次讀取，而檔案系統可以在檢查與 store 實際開檔之間改變。要關閉需要在
  每個 store 用 `O_NOFOLLOW` 與目錄 fd 相對開檔——那是「怎麼開檔」的改動，不是「怎麼判斷路徑」的
  改動。審查員另建議拒絕世界可寫的父目錄與既有異主檔案，同屬該層。
- **`#seatRoomMark` 與 `#seatWroteSince` 吞掉讀取錯誤**（見 ADR-043）；`hasAuthorMessageAfter`
  改為對未知 room 拋錯之後，那個 catch 又多吞一種錯誤。
- 錯誤訊息目前不回顯路徑，但若未來加上，需區分本機診斷與回給遠端 MCP 的回應。


## ADR-045：把宣稱交給會執行的東西去驗，不要交給會同意的東西

**狀態：** 已實作（2026-09-04）。審查第二輪十三條發現：七條已關閉、一條明確不修並記錄、五條
歸入其他 ADR 或已由本輪的量測方式取代。

**問題。** 審查第二輪的結論是「否，尚不足以公開上線」，而它列出的新問題有一個共同形狀：**測試
通過，並且替一個錯誤的宣稱背書**。

最具代表性的是能力矩陣測試。它為了回應第一輪「未證明其他工具不依賴 inbox」而寫，寫完通過了，
於是「21 個工具不受影響」這句話被寫進 ADR、「4 個工具停用」被寫進使用者訊息。兩個數字互相矛盾，
而且**都不對**。

那支測試的做法是：讀 `collab-server.ts` 的原始碼字串，在每個 handler 的內文裡找 `#seatInbox()`。
四層問題疊在一起：

1. **量的東西與被量的行為沒有因果關係。** `#seatInbox()` 只在完全沒有 collaboration 時才丟錯；
   真正會因為收件匣不可用而丟錯的是 `CollaborationService` 的 `inbox` getter，在另一個檔案。
   現成反例：`#noteUndeclaredSeatAction` 呼叫了 `#seatInbox()`，卻完全不碰 store。
2. **切片法追不了呼叫。** `room_send`、`room_await_reply`、`list_agents` 都是經由服務層方法碰到
   收件匣，MCP 層的原始碼裡看不到任何相關字樣。
3. **母體與樣本不一致。** 同一支測試斷言 dispatch 解析出 27 個工具，對照表卻只列 17 個，另外
   10 個從未被檢查。
4. **斷言是單向的。** 它只檢查「訊息裡提到的那 4 個有出現」，不檢查「該提的都提了」。

**決定：改成差分執行。** 建兩個世界，除了其中一個的收件匣被蓋上這個 build 不認得的 schema 之外
完全相同；用同一組參數把 27 個工具在兩邊各跑一次；「這個工具需要收件匣」的定義改成**降級世界回報
`ROOM_INBOX_UNAVAILABLE` 而健康世界沒有**。

第一次執行就否決了文件：實測停用的是 **7 個**，不是 4 個。多出來的正是那三個間接依賴。

**決定：清單只有一個來源。** `INBOX_DEPENDENT_TOOLS` 匯出，使用者訊息由它組出，測試拿它與實測
結果比對。三者只能一起對或一起錯。突變驗證涵蓋兩個方向：從清單移除一個真的會壞的工具會紅
（漏列），加入一個不會壞的也會紅（過度宣稱）。**後者才是新的**——舊做法下「多列幾個比較保險」
不會被任何東西擋住。

**兩個無法用錯誤差分判定的工具被明白標示，而不是默默算成不依賴。** `compare_agents` 把每個 target
的失敗吞進 `answers[].error`，永遠不會拋錯；`room_join_request` 把 briefing 包在 try/catch 裡，
降級時損失一個欄位而不是一次呼叫。前者在測試裡明確排除並說明理由，後者的依賴改在服務層量測。

**決定：`realpathSync` 只吞 ENOENT。** 原本的 bare catch 把所有錯誤當成「路徑還沒建立」，於是
EACCES、ELOOP、ENOTDIR、以及網路磁碟區的 EIO／ESTALE 都會讓迴圈一路往上走到 `/`——而
`realpathSync("/")` 一定成功，`canonical` 因此退化成純 `resolve(value)`，**也就是這個函式當初要
取代的字串版本，symlink 解析整個消失**。不需要有人攻擊，一次瞬時 I/O 錯誤就夠。

原本的測試證明不了這件事：它斷言 `/var/db/x` 與 `/private/var/db/x` 都被拒，但拒絕清單同時列了
兩種拼法，**把 `realpathSync` 整行刪掉，那兩個斷言照樣全綠**。新測試建一個位於 `/tmp` 底下、名字
不在任何清單裡的 symlink，指向 `/private/var/db`——**能拒絕它的只有跟著連結走這件事**。

**決定：建構子失敗時關閉已開的 store。** 九個 store 在建構子裡開啟，任何一個失敗，先前開好的
全部留著：throw 逃出建構子，沒有實例，沒有人叫得到 `close()`。每個 store 都會清理自己失敗的
建構，所以這個洩漏一直看不見——它發生在 store 之間。測試用可觀察的後果驗證：WAL 模式下乾淨關閉
會 checkpoint 並清掉 `-wal`，洩漏的連線不會。

**決定：失敗分類，但不回吐原始訊息。** 非 schema 錯誤原本原樣拋出，等於把「錯誤地降級」換成
「不可操作地啟動失敗」。現在分成 PERMISSION／CORRUPT／DISK_FULL／UNRECOGNISED 四類，各給一句
下一步。分類**先比對產品自己的錯誤碼，再才比對 SQLite 的英文散文**，因為那些散文會隨版本改變，
不能成為安全決策的依據；而分類結果不參與「是否降級」的判斷，所以猜錯只會讓句子沒那麼有用，
不會讓拒絕變弱。訊息刻意不含原始錯誤內容——fs 錯誤自帶絕對路徑，而這個字串會經 MCP 回傳並由
`doctor` 印出。

**明確不修，並說明理由：TOCTOU。** 路徑解析是一次讀取，檔案系統可以在檢查與 store 開檔之間改變。
關閉它需要每個 store 改用 `O_NOFOLLOW` 與目錄 fd 相對開檔——那是「怎麼開檔」的改動，會動到所有
store 的開檔路徑。本輪不做，也不宣稱已緩解。

審查員另指出一個相關但更具體的窗口：驗證通過後，`createAppContext` 只算一次資料目錄，之後整個
行程不再驗證。所以一個當下因 EACCES 而不可用的 symlink，若父目錄權限日後被放寬，寫入就會沿著
它落到別處。這條隨 TOCTOU 一起留著。
