...
- 排隊有上限：每席 32 筆未領取，超過即 `ROOM_INBOX_SEAT_LIMIT_REACHED`。GUI 與 MCP 兩側都寫出這個界線。
...
- 逾時說明同時讀「投遞狀態」與「席位是否在收聽」。只從後者推論會得出「沒人在聽所以沒人領走」，
...
給 Owner 自己的一筆有時間戳的稽核紀錄。**不是**「讓對方知道」——沒有任何機制保證這件事：
`room_wait` 與 `room_join_request` 的回傳都沒有帳本尾段，而給 agent 的指引是等待結束就再呼叫
`room_wait`，不是 `room_read`。按鈕文案因此明說「不保證它會去讀帳本」，並指向真正有效的動作：
直接交辦（會進收件匣排隊，席位下次待命就會拿到；除非它先被移出房間，或該席位收件匣已滿 32 筆而遭拒）。
