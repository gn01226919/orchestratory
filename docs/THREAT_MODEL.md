...
| T05 | 無限 loop 消耗訂閱/API 額度 | Hard limits、quota reservation、timeout、circuit breaker | Provider 計量延遲可能造成小幅超額 |
...
| T30 | GUI 將新 provider call 冒充成既有外接終端、把只在線的席位誤報成可喚醒，或受控即時 Agent 藉對話擴權寫檔 | 外接終端與 managed-subagent 使用分離 endpoint/seat ID；`room_join_request` 保持至 GUI 核准並開始第一段 bounded wait，之後只由原終端的 active `room_wait` 收件；API/UI 明示 `active-tool-pull`、`wakeable` 與休班 queued 狀態；直接 managed wake 固定唯讀。外接 Writer 另標示 via Writer Companion，HMAC audit 保存雙重身份；寫入只經 Writer Lease | 受控即時 Agent 每次直接喚醒不是 provider 原生持續 session；MCP 無法替完全 idle 的外部 host 發起新 turn；Writer Companion 代表執行而非原生終端程序，UI 與文件不得模糊三者 |
