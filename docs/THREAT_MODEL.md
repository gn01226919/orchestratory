...
- Promotion 紀錄自 2026-08-07（Phase 5-5 第六輪）起也有 CLI 出口：`orchestrator candidates promotions <workspace>` 列出每一筆促進在等什麼（**2026-08-07 第七輪更正：原文寫「唯讀列出」，實際會重新觀察並更新未結的紀錄**——那是標準第 13 項要求的收斂，錯的是形容詞，不是行為），`… release <promotion-id> --confirm <phrase> [--pid N] [--pgid N]` 是三個釋放動作的入口。**釋放不殺任何程序、不寫入 main、也不決定那次促進的結果**，改變的只有「這筆紀錄還在不在等某個 pid」；讀不了的列在還有程序活著時要求另一句說出「main 可能正在被寫」的短語，並要求引用記錄上的確切 pgid。`promoteMainMerge` 仍然刻意沒有任何產品側出口。
