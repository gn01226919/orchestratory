...
| Room macOS composer 鍵盤操作 | 已驗證（程式層／HTTP static contract＋Chrome double-enter） | 帳本直播與辦公室 textarea 共用 `installMacComposerKeyboard`；Enter 第一次保留換行、1.6s 內第二次送出；Shift/Option 換行、Command+Enter 立即送出；IME composition/229、repeat、內容／caret 變化、blur、disabled submit 防誤送；`test/web.test.ts` 覆蓋 HTML/JS contract；2026-07-22 Chrome 實測第一次 Enter 保留內容並新增換行、第二次 Enter 送出且只新增一則帳本訊息 | Browser 控制無法切換 macOS 系統輸入來源，仍需 owner 用實際中文 IME 驗收 composition Enter；可視鍵盤、語音輸入與 assistive technology 可繼續使用送出按鈕 |
