...
| MCP terminal presence／GUI membership／session-scoped standby／wake truthfulness | synthetic 已驗證；live smoke 僅涵蓋 2026-07-23 的 bounded 行為（2026-09-02 移除預設上限與 newest-wins 位移目前只有 synthetic 覆蓋，尚無新的 Chrome 實測） | owner-only SQLite schema v5、per-process UUID、canonical workspace exact match、5s heartbeat/15s lease、EOF unregister；`room_join_request` 只建立 GUI membership 申請，加入後 `room_wait` 另建立 exact-session 待命申請；Owner 核准後只有 active long-poll 是 `wakeable: true`，待命未核准時新交辦 fail closed，核准但 inactive 時 UI 誠實標示不可即時喚醒；待命 wait 預設沒有時間上限（2026-09-02 ADR-027 修正，五輪審查後結案；只有明確傳入的 `timeoutMs` 受四小時上限約束）；租約列在過期後保留 24 小時（`WAIT_LEASE_GC_MS`）而非即時清除，因為該列是「client 最後開的是哪一個等待者」的唯一紀錄，刪除它會讓被位移的舊迴圈搶回席位；`#claim` 的 waiter-token 守衛要求該列存在且相符，Owner 撤銷、MCP cancellation、EOF、presence 心跳停止（`canContinue` → `ROOM_STANDBY_REVOKED`）、`DELIVERY_LEDGER_MISMATCH`，或同席位新的 `room_wait` 接手（newest-wins，舊迴圈以 `ROOM_WAIT_LEASE_LOST` 結束且不得認領訊息）都會結束；**15 秒滾動 lease 失效不會結束待命**——它只是可見度訊號（`isListening`），過期列刻意保留 24 小時而非即時清除，否則兩個停滯的等待者都找不到自己的列，先醒的那個會把席位從 client 真正在讀的那個手上搶回去；無 provider fallback／替身。`test/room-presence.test.ts`、`test/room-inbox.test.ts`、`test/collab-mcp.test.ts`、`test/collaboration-service.test.ts`、`test/web.test.ts`。2026-07-23 Chrome 在 `orchestratory` 精確 Room 實測：加入申請→GUI membership 核准→`room_wait` 待命申請→GUI 待命核准→`wakeable`→GUI `@codex6` 精確收件／ack／回覆 `GUI_ROOM_WAIT_OK`→回覆後重新待命；GUI 撤銷立即令 open wait 回傳 `ROOM_STANDBY_REVOKED`，終端 stdio 關閉後 5 秒內人物與授權自動移除。2026-07-22 另已驗證跨專案切換不顯示該席位 | 移除預設上限之後，實際的邊界改由 MCP client 自身的 request timeout 決定，Orchestratory 無法觀測或延長它；任何 client-side 結束都必須由 Agent 再次呼叫 `room_wait`。完全 idle 的既有 host 不能接受 server-initiated turn；真正不依賴 external pull 的 GUI wake 使用分離的受控即時 Agent。既有 MCP child 必須由 host 重開才會載入新 tool schema |
...
| MCP 精確席位收件匣 | 已驗證（synthetic＋跨程序） | `queued→delivered→read→working→replied/expired/failed/cancelled`（`expired`＝掃描當下仍為 `queued`，且距離最後一次「人為要求」（`enqueue`／`owner-retry` 事件）已超過 12 小時；**不要求從未被領取**（WORDING-EXCEPTION：這是否定句，正是本規則的重點）——被領走又因 lease 到期退回佇列的交辦同樣會過期，有測試覆蓋。狀態變更而非刪除；`cancel` 與 `waitForReply` 視為終局並有測試覆蓋，`fail` 的同款守衛屬 defence in depth 且刻意未測：過期列必為 `queued`，而目前所有進入 `queued` 的轉移都清空 lease token，故 `#authorizedLease` 先擋下——這是實作慣例，不是 schema 保證（schema 只約束 lease 兩欄同進同出）。`expired` 可由 owner 重新排隊，並取得新的計時起點）、`room_wait/ack/reply/fail`、私有 lease token、斷線不誤耗重試、已讀後 bounded retry、取消、離線 fail、idempotent crash recovery、無 provider fallback；schema v4 保存 authenticated source/thread/reply-to，v3 既有 row 交易遷移並以 delivery ID 建立 legacy thread；`test/room-inbox.test.ts`、`test/collab-mcp.test.ts`、`test/collaboration-service.test.ts` | 交辦過期由 `roomView` 驅動（本產品無背景排程器），所以**沒有人開啟的房間會保留它的積壓**直到有人查看；同一筆交辦第二次過期不會再留帳本行（冪等 key 與文字皆相同，走 replay 分支），帳本讀起來會像只發生過一次；真實外接 CLI 仍必須在每次結束後自行再次呼叫 `room_wait`（不再是因為四小時上限，而是因為 client 自身 timeout 或回覆會結束該次呼叫）；待命未核准時新交辦拒絕，核准但沒有 active wait 時 GUI 只標示未值班，不宣稱即時喚醒；已安裝 MCP 尚未切換 vNext |
...
### 2026-09-05 六條 UI 線與 P0-3 併入後的合併對話框驗收

Owner 在真實瀏覽器操作以 candidate 自己的 `src/` 與 `public/` 啟動的 loopback server
（`127.0.0.1:63925`），資料目錄與 repository 全部新建於暫存目錄，**未接觸 Owner 的正式資料
目錄**。預覽內容是以產品自己的 `CandidateRegistry` 建立的**真實**待核准紀錄，不是靜態假資料
——否則這次驗收釘住的就只是「畫面長這樣」，不是「產品在真實狀態下畫出的畫面」。

被驗收的畫面包含七批改動：P0-3 的未經證實 promotion 區塊（`485f4e7`）、C 線右欄 rail
（`c2768bb`）、B 線舞台（`8d32255`）、D 線核准層與 `.app` 版面（`0aed842`）、E 線整合接線
（`8311542`）、F 線頂欄切換鈕與常駐按鈕（`107abee`），以及 G＋H 的 🔔 核准鈕、紅色提示條、
席位更名與拒絕加入（`ddcc62c`）。

| 行為 | Owner 的實際觀察（逐字） |
|------|--------------------------|
| 頂欄切換鈕與常駐按鈕 | 「ok 都在」——辦公室／帳本切換鈕存在，「⑂ 待核准 N」與「▤ 併入紀錄」常駐且看得清楚 |
| 四區分色與 P0-3 區塊 | 「有，做得不錯」——已 Merge 綠／需要檢查 黃／可重新發起 藍／不能重新發起 灰分得開，未經證實的 promotion 區塊有出現 |
| 是否有壞掉的地方 | 「還可以更好」——Owner 明確區分了這是優化空間，不是缺陷；後續優化以 I 線進行，該線不動雜湊內容 |
| 副作用邊界 | 全程唯讀：沒有 promotion、沒有核准、沒有 API/MCP、push、publish、deploy、delete 或 cleanup |

**這一輪的 digest 換過三次，而只有最後一次是被花掉的。** `d762cd43` 是 D 線 rebase 後的值，
在合併 C 與 B 之後量過三次都不變，本來要作為最終值；隨後 Owner 看了畫面提出「帳本在哪」與
「merge 紀錄要一目瞭然、大一點」，F 線因此改動 `merge-outcome-nav`（雜湊涵蓋的七段之一），
digest 移動到 `adab0c08`。**舊值從未被寫入，所以沒有任何一次驗收被浪費**——讓它失效的不是
合併，是一個真正該做的改動。

合併過程中 digest 量了六次而不是推論一次：併 C 後、併 B 後、併 D 後、F 線套用後、G＋H 套用
後、以及本次寫入前。理由是 `.app` 那條 regex 帶 `^  ` 前綴，三份版面改動落在同一個檔案的相鄰
區域，任何一方影響到縮排或前後順序都會讓它失準——而那會讓 Owner 驗收一個與最終 main 不同
的畫面，正是這個機制存在要防的事。

新的 served-bytes digest：`adab0c08754fe38191faf518032427176652fb00bfb077f8a063c1dd1b5bc6eb`。


---

## 2026-09-05 夜間 · 併入紀錄畫面（K 線）Owner 瀏覽器驗收

在上一次驗收（`adab0c08`）之後，`merge-dialog-acceptance` 涵蓋的內容因 K 線而改動：併入紀錄
對話框新增四組側邊目錄、把「已失效」拆成自己的分組、改寫 `promotion-attestation` 卡片的說法，
並把專案級確認鈕從分組底部的橫幅移到卡片本身。

**Owner 在 `127.0.0.1:4318` 的實機畫面上驗收，逐字紀錄：**

| 行為 | Owner 的實際觀察（逐字） |
|------|--------------------------|
| 整體 | 「這樣很不錯啊，已驗證，很好」 |
| 側邊目錄四組 | 畫面顯示 已併入 8 · 等你確認 0 · 已失效 12 · 未進入 0，四組可點選 |
| 已失效分組 | 出現「已失效：被拒絕」卡片，原因欄為白話（「這份工作已經進入 main 了（由另一次核准完成），所以不需要、也不能再併入一次。」），非錯誤碼 |
| 確認鈕位置 | Owner 先前在舊版找不到該鈕（它在 16 張卡片之後）；本版位於卡片上，Owner 按下後稽核鏈留下 `main-merge-acknowledge`（actor=you） |

**為什麼這個 digest 與交接時預告的 `cc52e8be…` 不同。** 同儕交接 K 線時量到的是 `cc52e8be`；
其後 Codex 逐項審查（8 項準則，第 1 輪 5/8、第 2 輪 8/8）指出三項不通過，修正三項都落在
`public/room.js`：未知 `refusal.code` 不再冒充說明、卡片按鈕移除一個恆為真的條件、以及移除
與卡片重複的底部橫幅（**橫幅獨有的五句揭露先搬進卡片才刪**）。**每一次改動都讓預告值失效，
這正是這個機制的用途**——digest 釘的是「有一個人看過現在這個畫面」，不是「有人看過某個版本」。

Owner 驗收的是本次寫入前量到的最終值，非任何中間值。

**副作用邊界**：本次驗收全程唯讀，唯一的寫入是 Owner 自己按下的 `main-merge-acknowledge`
（它不修復任何紀錄、不寫入 main、不結束任何程序，且只對當時那個服務程序有效）。

新的 served-bytes digest：`5588ec56f2a6c9b5dae0f6e8312925623bbab3fa1459c637411e9fb5bed58dbb`。
