...
| Supply chain/release gate | 已驗證（未發布 tree） | `npm run check:release`、SBOM、audit、secret/history scan、offline committed-HEAD clean clone；tracked package allowlist 排除 test/non-runtime scripts/CI/Agent instructions，實際 tgz 使用 runtime-only manifest，離線安裝後驗 `.bin` link/mode、JS/MJS syntax、TS typecheck、CLI startup 與正負向 audit smoke | artifact signing/provenance 需先有 owner 批准的 release |
| GitHub 開源發布 | 未批准 | release checklist 保持 NO-GO | 名稱、license、Git identity、remote、visibility、commit/push/release 均待 owner 決策 |

## 目前自動證據

- 273/273 deterministic tests。
- 最新最終 gate：line 95.00%、branch 85.15%、functions 96.81%；gate 分別為 90%、85%、90%。
...
- 完整 npm dependency audit（含 dev toolchain）0 vulnerabilities；offline committed-HEAD clean clone與
  88-file tracked-allowlist tgz 安裝／bin／audit reproduction 均通過；local/history scan 已覆蓋目前完整 history。
